...
import type { BoardPost, ColNode, Node, RowNode, Section } from "@/lib/types";

/**
 * Bespoke home notice ticker.
 *
 * Extracted from `app/[locale]/page.tsx` so the exact same markup can be reused
 * by the admin preview (see `SectionPreview`). The module is dependency-light
 * (no hooks, no server-only imports): it renders identically on the server page
 * and inside the client editor island. `Rows` comes from `SectionRenderer`,
 * which is already part of the client bundle via `SectionPreview`.
 */
 ...
 */
export function isNoticeTickerSection(sec: Section): boolean {
  const walk = (nodes: Node[]): boolean =>
    nodes.some((n) =>
...
export default function NoticeTicker({
  ...
  const headerRows: RowNode[] = section.rows.filter(
    (r): r is RowNode => r.kind === "row" && (r.h === 126 || r.h === 116 || r.h === 39),
  );
  const headerRow = headerRows.find((r) => r.h === 116);

  return (
    <section
      className="ticker-notice pc-at-mobile relative overflow-x-clip"
      style={{ backgroundColor: section.bgColor || "#f7f7f7" }}
    >
          ...
          r === headerRow ? (
            ...
            >
              {r.cols.map((c, j) => (
                ...
                >
                  <Rows rows={c.children} locale={locale} nested wdepth={1} />
                </div>
              ))}
            </div>
            ...
            <Rows key={i} rows={[r]} locale={locale} />
          ),
        )}
