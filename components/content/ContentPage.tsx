...
import { applySharedIntroRows, getResolvedPage } from "@/lib/content/resolved";
...
export default async function ContentPage({
  ...
  // Shared intro band: the canonical channel page's copy is the single source,
  // and every other page renders ITS rows in place of its own dead copy. The
  // swap helper replaces the rows in place (never the section) so each page
  // keeps its local `id`/`cls`/`bg` (company.philosophy's band id is in
  // PH48_SECTION_IDS, so swapping it would regress that page's mobile
  // typography); a channel root that aliases the canonical page already serves
  // the canonical copy and is left alone. See `applySharedIntroRows`.
  const withSharedIntro = await applySharedIntroRows(l, pageKey, all);

  const dropped = new Set<string>();
  const firstPc = withSharedIntro.find((s) => !isMobileSection(s));
  const firstMobile = withSharedIntro.find(isMobileSection);
  ...
  const sections = withSharedIntro.filter((s) => !dropped.has(s.id));
